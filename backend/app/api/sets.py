...
def get_standard_blocks() -> List[StandardBlock]:
    ...
    cursor.execute(
        """
        SELECT id, name, releasedate, laststandarddate 
        FROM sets WHERE laststandarddate IS NOT NULL
    ...
    for result in results:
        if result[3] == current_block_date:
            set_context.append(Set(id=result[0], name=result[1], last_standard_date=result[2], release_date=result[3]))
        else:
            output.append(StandardBlock(last_standard_date=current_block_date, sets=set_context))
            current_block_date = result[3]
            set_context = [Set(id=result[0], name=result[1], last_standard_date=result[2], release_date=result[3])]
    output.append(StandardBlock(last_standard_date=current_block_date, sets=set_context))
    return output
